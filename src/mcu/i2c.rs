...
fn get_current_address(p: &nrf52832_pac::Peripherals) -> u8 {
	p.TWI0.address.read().bits() as u8
}

fn setup(p: &nrf52832_pac::Peripherals, i2cline: &I2cLine){
	let i2c = &p.TWI0;
...
#[allow(dead_code)]
pub fn write_byte(p: &nrf52832_pac::Peripherals, i2cline: &I2cLine, byte: u8, send_start: bool, send_stop: bool) -> Option<bool>{
	if get_current_address(p) != i2cline.address {
		setup(p, i2cline);
	}
...
#[allow(dead_code)]
pub fn write_data(p: &nrf52832_pac::Peripherals, i2cline: &I2cLine, data: &[u8], send_start: bool, send_stop: bool) -> Option<bool> {
	if get_current_address(p) != i2cline.address {
		setup(p, i2cline);
	}
	...
	for i in 0..data.len() {
		match write_byte(p, i2cline, data[i], false, send_stop && (i == (data.len() - 1))) {
			None => return None,
			_ => ()
		};
	}
...
#[allow(dead_code)]
pub fn read_byte(p: &nrf52832_pac::Peripherals, i2cline: &I2cLine, send_stop: bool) -> Option<u8> {
	if get_current_address(p) != i2cline.address {
		setup(p, i2cline);
	}
